//! zernus_render — Zeronus 原生图片渲染扩展（pyo3 + fontdue + image）
//!
//! 提供两个函数，输出 PNG bytes：
...
//!
//! 使用 abi3（稳定 ABI）编译：Windows 产出 zernus_render.pyd，Linux 产出 zernus_render.so，
...
#[pymodule]
fn zernus_render(m: &Bound<'_, PyModule>) -> PyResult<()> {
