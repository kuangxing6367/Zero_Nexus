# zernus_render — Zeronus 原生图片渲染扩展
...
└── bin/                # 编译产物（插件按平台自动加载）
    ├── win64/zernus_render.pyd         # Windows x64
    ├── win32/zernus_render.pyd         # Windows x86 (32 位)
    ├── linux-x86_64/zernus_render.so   # Linux x64
    ├── linux-aarch64/zernus_render.so  # Linux ARM64
    ├── linux-i686/zernus_render.so     # Linux x32 (i686)
    ├── linux-armv7/zernus_render.so    # Linux ARMv7 (armhf)
    └── linux-loongarch64/zernus_render.so  # 龙芯 LoongArch64
```
...
> `.github/workflows/build-zernus-render.yml` 自动构建并通过 ELF/PE 架构自校验；
...
推送后由 `.github/workflows/build-zernus-render.yml` 自动构建，也可手动触发：

```bash
# Windows / Linux 各出一个 artifact，内含 zernus_render.pyd / zernus_render.so
gh workflow run build-zernus-render.yml
gh run watch          # 等待完成
gh run download <run_id> --pattern 'zernus_render-*'
```

下载解压后放入对应平台目录：

- `native/bin/win64/zernus_render.pyd`（Windows x64）
- `native/bin/linux-x86_64/zernus_render.so`（Linux x64，aarch64 用户放 `linux-aarch64/`）
...
# 从 target/wheels/*.whl 中取出 .pyd / .so，重命名为 zernus_render.pyd / .so
