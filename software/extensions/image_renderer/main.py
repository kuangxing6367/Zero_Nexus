...
渲染引擎自动选择（按平台）：
  1. 原生扩展 zernus_render（Rust + pyo3，放在 native/bin/<平台>/）
...
def _load_native_renderer():
    ...
    if sys.platform.startswith('win'):
        ...
        names = ['zernus_render.pyd', 'zernus_render.abi3.pyd']
    elif sys.platform.startswith('linux'):
        subdirs = _linux_native_subdirs((platform.machine() or '').lower())
        names = ['zernus_render.so', 'zernus_render.abi3.so']
    else:
        return None
    tried = []
    for sub in subdirs:
        for name in names:
            ...
            if not os.path.isfile(path):
                continue
            try:
                spec = importlib.util.spec_from_file_location('zernus_render', path)
...
class _NativeCanvasProxy:
    ...
    原生 zernus_render.Canvas.text(x, y, ...) 的 y 是基线语义（内部 baseline = y + font_size），
    ...
    def text(self, x, y, text, font_size=20, color=None, align="left", wrap_width=0):
        ...
        背景：原生 zernus_render（fontdue 0.9）的 draw_text 曾误用 ymin 符号
