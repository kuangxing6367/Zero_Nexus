...
鉴权/CORS/限速/辅助函数/约 180 个端点全塞在一个闭包里。这里把它拆成：
    ...
    graphql.py      GraphQL 查询/变更端点（/api/graphql）
    grpc.py         gRPC 服务状态接口（/api/grpc/info）
...
# 项目根目录：统一走内核路径工具。历史上此处用「向上三级」硬算，
# 迁到 software/extensions/webui/ 后只到 software/，导致前端 404、VERSION/.git 读错。
from core.kernel.paths import project_root as _kernel_project_root

_PROJECT_ROOT = _kernel_project_root()
...
def create_web_app(framework) -> Flask:
    ...
    # ---- 通讯安全校验（Token / RSA 回调） ----
    auth_sys = AuthSystem(framework.config.get('security', {}), db=db)

    @app.before_request
    def _global_blacklist_guard():
        """全局黑名单拦截：被封禁的 IP 无法访问任何路由（白名单除外）"""
        ip = get_client_ip()
        if auth_sys.is_whitelisted(ip) or not auth_sys.is_blacklisted(ip):
            return None
    ...
    _DEFAULT_MARKET = {
        'name': 'Zeronus 官方插件源',
        'url': 'https://raw.githubusercontent.com/kuangxing6367/Zero_Nexus_plugins/main/registry.json',
    }

    _MIRROR_MARKETS = [
        {
            'name': 'Zeronus 镜像源 (ghproxy)',
            'url': 'https://ghproxy.net/https://raw.githubusercontent.com/kuangxing6367/Zero_Nexus_plugins/main/registry.json',
        },
        {
            'name': 'Zeronus 镜像源 (ghproxy.cn)',
            'url': 'https://ghproxy.cn/https://raw.githubusercontent.com/kuangxing6367/Zero_Nexus_plugins/main/registry.json',
        },
    ]
    ...
    def _download_plugin_tree(repo: str, branch: str, sub_path: str, target_dir: str):
        ...
        try:
            ...
            headers = {'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'zernus'}
            tree = None
            api_err = ''
    ...
    # 注：graphql 依赖可选第三方（ariadne），单独在下方防御式导入并挂载。
    from . import (
        ...
        static_routes, batch, webhook, grpc,
    )
    auth.register(ctx)
    admins.register(ctx)
    ...
    batch.register(ctx)
    webhook.register(ctx)
    grpc.register(ctx)
    # GraphQL 域依赖 ariadne（可选）：缺依赖时跳过该域，不影响 Web 后台其余功能
    try:
        from . import graphql as _graphql_mod
        _graphql_mod.register(ctx)
    except Exception as e:
        logger.warning(f"GraphQL API 域未挂载（可选依赖 ariadne 缺失？）: {e}")

    # ---- 插件自定义 API 路由挂载（framework.api.registry）----
    from . import registry as _api_registry
