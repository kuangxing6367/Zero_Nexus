...
**当前版本：v0.0.1-beta.0-alpha.0** ｜ 版本演进见 [CHANGELOG.md](CHANGELOG.md)

- 项目地址：https://github.com/kuangxing6367/Zero_Nexus
...
Zeronus 是一个**微内核式的通用服务宿主**：极小内核（加载、路由、公共服务、扩展点契约）+ 层层叠加的扩展。
所有具体功能（接入平台、开网页后台、管会话、跑定时、做权限）都是可插拔的扩展，通过内核的**扩展点（hook）**
挂到几乎每一个运行环节。

### 它适合谁（三类目标用户）

| 你是…… | 你能用 Zeronus 做什么 | 从哪里开始 |
| ------ | ------------------- | ---------- |
...
- **最小核心（`core/`）**：插件加载器、事件总线、消息路由、服务注册表（DI）、**扩展点注册表（HookRegistry）**、
  权限引擎、数据库抽象、运行时上下文。**不实现任何具体业务，也不含任何 OneBot 代码**。
- **官方扩展（`extensions/`）**：随项目提供的基础能力，开关与配置集中在根目录 **`extensions.yaml`**，按需加载。
...
> 提示：想要**纯 托管机器人**？什么都不用关，开箱即用。想要别的形态？在 `extensions.yaml` 里切换接入端、换一套业务插件即可，权限、后台、持久化、会话这些骨架原样保留。

---

## 二、内核与扩展层

Zeronus 的能力分两层：**内核只负责"运转"**，其余都是"挂在运转各环节上的扩展"。

| 层 | 内容 | 属于 |
| ---- | ---- | ---- |
| **内核（core/）** | 加载器 · 事件总线 · 消息路由 · 服务注册表(DI) · **扩展点注册表** · ctx · 权限引擎 · 数据库抽象 · 运行时上下文 | 微内核（不可关） |
| **公共服务扩展（extensions/，可开关）** | Web 后台 / 接入端 / 会话 / 定时 / HTTP 注入 / 独立 API | 官方扩展 |
...
```
┌──────────────────────────────────────────────────────┐
│              core/      微内核（零 OneBot 代码）        │
│  加载器 · 事件总线 · 消息路由 · ctx · 服务注册表(DI)     │
│  扩展点注册表(HookRegistry) · 权限引擎(perm) · 数据库(db) │
│                       │ 扩展点 / 服务注册 / 事件总线      │
└───────────────────────┬──────────────────────────────┘
                        │ 启动时按 extensions.yaml 加载扩展
        ┌───────────────┼───────────────────────┐
        ▼               ▼                        ▼
┌────────────────┐ ┌────────────────┐      ┌────────────────┐
│ onebot_adapter │ │ webui          │      │ scheduler/session│  extensions/（可开关的扩展）
...
聊天平台官方通常不提供"把账号变成机器人"的接口，社区用 **OneBot 实现端**接管一个账号，再把消息转发给 Zeronus。

- **WebSocket（WS）**：一条常驻连接，消息即时推送，类似"打电话"而非"发短信"。
- **反向连接**：Zeronus 当**服务端**（在 `6830` 端口等），OneBot 端当客户端主动连进来，所以叫"反向"。你只需在 OneBot 端填对地址。
...
```bash
python main.py                 # 也可指定配置：python main.py D:\config\zernus.yaml
```
...
首次启动会自动生成全局配置 `config.yaml`、官方扩展配置中心 `extensions.yaml` 与 `data/` 目录。

<details>
<summary><b>config.yaml 和 extensions.yaml 怎么分工？</b>（点我展开）</summary>

- **`config.yaml`**：框架全局设置——数据库、日志、安全、插件目录等。
- **`extensions.yaml`**：**官方扩展的开关与配置中心**。启动时自动扫描 `extensions/`：新装扩展补块、卸载扩展删块、自动回写，并合并进主配置（所以代码里 `fw.config.get('onebot')` 这类读法不变）。

常用默认端口/账号：

| 配置（编辑 `extensions.yaml`） | 默认值 | 作用 |
| ------ | ------ | ---- |
...
| `config.yaml → database.type` | `sqlite`（`data/zernus.db`） | `sqlite` 零配置或 `mysql` |
...
| AccessToken | 与 `extensions.yaml → onebot_adapter.access_token` 一致（本地调试可留空） |
...
[插件文档](https://github.com/kuangxing6367/Zero_Nexus_plugins/blob/main/plugins/llm_plugin_gen/docs/INDEX.md)。

### 4. 不接 IM：用 HTTP 注入事件（内置 `http_inject`）

在 `extensions.yaml` 把 `http_inject.enabled` 改为 `true` 并重启，外部系统 POST 一条 JSON 即可注入事件，业务扩展照常处理——接 GitHub/支付回调、搭内部工具都不需要 IM：
...
在 `extensions.yaml` 关掉 `onebot_adapter`、保留 `scheduler`，用 `ctx.task()` 注册定时任务，就变成一个纯定时宿主。要接 Telegram/Discord/MQTT 等，写一个 `ProtocolAdapter` 即可（见[协议适配器](docs/api/advanced/protocol_adapter.md)），权限/后台/持久化能力不变。
...
把 `config.yaml → plugin.dir` 指向 [Zero_Nexus_plugins](https://github.com/kuangxing6367/Zero_Nexus_plugins) 仓库，或在后台插件市场安装。常用：
...
# require_level='admin'  ≡  检查节点 zernus.role.admin
```

后台「权限管理」页（`/permissions`）含权限组 / 用户 / 轨道 / 校验器 / 审计 5 个标签页，所有变更写入 `perm_audit`。节点命名建议：业务用 `插件名.动作.子项`，内置身份用 `zernus.role.*`。完整机制见[权限系统文档](docs/advanced/permission.md)。

---

## 十、接口令牌（API Key）与两类 HTTP 接口

Zeronus 有**两类** HTTP 接口，别混淆端口：
...
├── extensions.yaml       # 官方扩展配置中心（启动自动扫描 extensions/ 同步、回写、合并）
├── requirements.txt
...
│   ├── config/             # 配置加载 + extensions.yaml 配置中心
│   ├── hooks/              # 扩展点注册表（HookRegistry）—— 内核契约
│   ├── storage/            # 数据库抽象（SQLite/MySQL 双方言、连接池、自动建表）
│   ├── messaging/          # Event 事件对象 / 事件总线 / 消息路由
│   ├── ctx/                # PluginContext（扩展可用能力，含 ctx.hook 扩展点注册）
│   ├── commands/           # 命令原语 CommandBus（register + invoke）
│   ├── adapters/           # ProtocolAdapter 抽象基类 + ActionProxy + 服务注册表
│   ├── perm/               # LuckPerms 风格权限引擎（无第三方依赖）
│   ├── plugin_loader/      # 用户扩展加载器（合成包/相对导入/可靠热重载）
│   ├── zkg/                # 包管理器（依赖驱动加载 + 多源索引）
│   ├── auth/               # 认证（会话 token + API Key）
│   ├── scheduler/          # 定时任务调度
│   ├── terminal/           # 终端交互
│   ├── tls/                # SSL / TLS 上下文
│   ├── log_broker/         # 日志代理（控制台 + 轮转文件 + 内存日志流）
│   ├── cache.py / di.py    # TTL 缓存 / 依赖注入容器
│   ├── webhook.py          # 出站 Webhook 分发
│   ├── api/                # 后台 REST 功能域分包（可插入路由注册表）
│   └── ipc/                # core/host 双进程 JSON-RPC（dual_process 门控）
├── extensions/             # 官方扩展（可在 extensions.yaml 开关）
...
- **缓存**：权限解析 60s TTL、组快照 10s，Web 端改动后自动失效。底层能力都在 `core/perm/`。
