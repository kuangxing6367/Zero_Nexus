...
class Session:
    ...
    async def wait(self, timeout: float = None) -> dict:
        ...
        hooks = self._framework.hooks
        await hooks.trigger_async(HookPoints.SESSION_WAIT_BEFORE, key=self.session_id, timeout=t)
        loop = asyncio.get_running_loop()
        self._future = loop.create_future()
        self._expires = time.time() + t
        try:
            result = await asyncio.wait_for(self._future, t)
        except asyncio.TimeoutError:
            await hooks.trigger_async(HookPoints.SESSION_WAIT_AFTER, key=self.session_id, result=None)
            return None
        await hooks.trigger_async(HookPoints.SESSION_WAIT_AFTER, key=self.session_id, result=result)
        return result
...
class SessionManager:
    ...
    async def wait_for(self, ctx, event, prompt=None, timeout=_DEFAULT_TIMEOUT, handler=None):
        ...
        await hooks.trigger_async(HookPoints.SESSION_WAIT_BEFORE, key=key, ctx=ctx, event=event, timeout=timeout)
        loop = asyncio.get_running_loop()
        ...
        try:
            ...
            await hooks.trigger_async(HookPoints.SESSION_WAIT_AFTER, key=key, ctx=ctx, event=event, result=None)
            return None
        await hooks.trigger_async(HookPoints.SESSION_WAIT_AFTER, key=key, ctx=ctx, event=event, result=result)
        return result
    ...
    def create_session(self, ctx, event, timeout=60) -> Session:
        ...
        hooks = self.framework.hooks
        try:
            hooks.trigger_sync(HookPoints.SESSION_CREATE_BEFORE, key=key, ctx=ctx, event=event, timeout=timeout)
        except Exception as e:
            logger.error(f"扩展点 [session.create.before] 异常: {e}")
        sess = Session(key, self.framework, event, timeout)
        try:
            hooks.trigger_sync(HookPoints.SESSION_CREATE_AFTER, key=key, session=sess)
        except Exception as e:
            logger.error(f"扩展点 [session.create.after] 异常: {e}")
        return sess
