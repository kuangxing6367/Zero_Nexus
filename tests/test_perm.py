...
from core.storage import Database, _auto_create_tables
...
print("\n== 内置角色组 ==")
for role, node in (('member', 'zernus.role.member'), ('admin', 'zernus.role.admin'),
                   ('owner', 'zernus.role.owner'), ('super', 'zernus.role.super')):
...
chk("super 继承得到 zernus.role.admin", p_super.has('zernus.role.admin'), True)
chk("super 继承得到 zernus.role.member", p_super.has('zernus.role.member'), True)
p_admin = perm.resolve(db, UID, {}, 'admin', use_cache=False)
chk("admin 不持有 zernus.role.owner", p_admin.has('zernus.role.owner'), False)
