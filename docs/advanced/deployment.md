...
ExecStart=/opt/zernus/.venv/bin/python /opt/zernus/main.py
Restart=always
RestartSec=5
# 安全加固（按需）
...
sudo systemctl enable --now zernus
journalctl -u zernus -f          # 查看实时日志
```
...
群多、并发高时把 `database.type` 切到 `mysql`，提前建库（如 `zernus`，
`utf8mb4`），框架会自动适配 DDL 并建表；需要 `pymysql`、`DBUtils`
（切换时会提示/自动安装）。

## 安全清单

1. 在 `extensions.yaml` 给 `onebot_adapter.access_token` 设强随机令牌，OneBot 客户端保持一致；
