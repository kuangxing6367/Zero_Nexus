...
本文讲清楚 Zeronus 到底是怎么把 `plugins/xxx/main.py` 变成一个运行中插件的，
...
用户插件由 `core/plugin_loader` 的 `PluginLoader` 管理，完整生命周期如下：
...
启动顺序（`core/engine.py → Framework.start()`）：

1. 先加载 `extensions/` 官方插件（提供协议适配、调度器、会话等基础服务）；
...
```python
from core.ctx import ...          # 框架代码：正常绝对导入（项目根在 sys.path）
...
相关方法均在 `core/plugin_loader` 的 `PluginLoader`：
