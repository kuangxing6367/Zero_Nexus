...
> 定位：Zeronus 是一个**事件驱动的插件化服务宿主**，OneBot只是默认接入端之一。
> **本篇面向**：角色 B/C，尤其是想把它用于非 IM 场景（纯定时 / HTTP Webhook / 内部后台 / 其它 IM）的开发者。
> 本页讲的是**把它当通用服务宿主用**的正确姿势——不局限于单一平台。

---

## 一、先想清楚：Zeronus 不是"插件化服务宿主"

很多人误以为 Zeronus 只能做 托管机器人。其实它的骨架是通用的：
...
`extensions/scheduler` 已经内置 APScheduler。把 `extensions.onebot_adapter` 关掉，只留调度器，就变成一个纯定时服务宿主：

```yaml
# extensions.yaml（官方插件配置中心）
extensions:
  onebot_adapter:
...
> 内置 `http_inject` 已实现等价能力（默认 `127.0.0.1:8901/hook`，在 `extensions.yaml` 开启即用）。下面手写一个最小 adapter，帮助你理解契约、自定义路径与鉴权；完整版见 [协议适配器](../api/advanced/protocol_adapter.md)。
...
from core.adapters import ProtocolAdapter
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        asyncio.run_coroutine_threadsafe(
            self.core.dispatch_event(event), self.core.loop).result()
