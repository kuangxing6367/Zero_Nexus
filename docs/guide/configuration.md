...
> **本篇面向**：角色 A/B（使用者与插件开发者）。Zeronus 有两份配置，分工如下：
> - **`config.yaml`**：框架**全局**配置（数据库、日志、安全、插件目录等），首次启动自动生成；
> - **`extensions.yaml`**：**官方插件配置中心**，集中管理 `extensions/` 下每个官方插件的开关与配置。
>
> 也可以在启动时传入自定义的全局配置路径：`python main.py /path/to/config.yaml`。

## 官方插件配置中心 extensions.yaml（重点）

官方插件（`extensions/` 目录）的开关与配置**统一写在根目录 `extensions.yaml`**，不再散落在 `config.yaml`。

启动时框架会自动完成同步（见 `core/config.py` 的 `_autoload_extensions`）：

1. 扫描 `extensions/` 目录，得到"已安装"的官方插件；
2. 为**已安装但 yaml 里缺失**的插件补上默认配置块；
3. **删除** yaml 里已经不再安装的插件块（即卸载插件后配置块自动消失）；
4. 有变化就**自动回写** `extensions.yaml`（无需手动维护）；
5. 把每个插件的 `enabled` 合并进主配置的 `extensions` 段，把插件配置块合并进对应主配置段
   （`onebot_adapter → onebot`、`webui → web`，其余插件段名与插件名相同）。

因此**代码里既有的 `fw.config.get('onebot')`、`fw.config.get('web')` 等读法保持不变**，对插件透明。

一份开箱即用的 `extensions.yaml` 形如：
...
> 提示：旧版本把开关写在 `config.yaml → extensions` 的写法仍能被识别（向后兼容，首次合并时迁移），
> 但新配置请统一写到 `extensions.yaml`。

## 协议接入（OneBot 11，来自 onebot_adapter）

`onebot_adapter` 的配置块在 `extensions.yaml`，合并后主配置段为 `onebot`：
...
Zeronus 作为 WebSocket **服务端**，由 NapCat / Lagrange 等 OneBot 实现端反向连接。
...
```yaml
# extensions.yaml → extensions.webui（合并后主配置段为 web）
web:
...
先在 `extensions.yaml` 把 `http_api.enabled` 置为 `true`：
...
log:
  ...
  file: data/logs/zernus.log   # 日志文件
