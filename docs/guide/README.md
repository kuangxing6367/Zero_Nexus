...
> **Zeronus 是一个事件驱动的插件化服务宿主**，内置依赖注入（服务注册表）、LuckPerms 风格权限引擎、
...
3. [配置系统](./configuration.md) —— `config.yaml` 与官方插件配置中心 `extensions.yaml`
...
| 文档 | 内容 | 主要受众 |
| ---- | ---- | -------- |
| [PluginContext (ctx)](../api/basic/ctx.md) | 插件可用能力的完整清单（含文件 / HTTP / 序列化 / 调度 / 事件 / 缓存 / 依赖注入） | B |
...
| 路径 | 作用 |
|------|------|
| `core/` | **极简内核**：加载器、路由、事件、上下文、协议抽象、权限、数据库（不含任何 OneBot 实现） |
| `extensions/` | 官方插件：`onebot_adapter` / `http_inject` / `http_api` / `webui` / `session` / `scheduler` / `image_renderer` |
| `extensions.yaml` | **官方插件配置中心**：开关与配置集中于此，启动自动扫描同步并合并进主配置 |
