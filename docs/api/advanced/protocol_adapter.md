...
> 这是让 Zeronus **不局限于单一平台** 的关键文档。框架核心不绑定任何具体 IM 协议（`core/`
...
```
外部事件源 ──► ProtocolAdapter（转换）──► core.dispatch_event(event) ──► 命令路由 / 事件总线 ──► 插件
```
...
`core/engine.py` 的 `dispatch_event` 按 `type` 分流，消息类事件至少需要这些字段：
...
> 对照实现可看 `extensions/onebot_adapter/main.py` 的 `OneBotAdapter.send_text`。
...
```
extensions/http_webhook/
├── __init__.py
└── main.py
```

> 放进 `extensions/` 就是官方插件（在 `extensions.yaml` 里用开关控制）；
> 放进 `plugins/` 就是用户插件。两者加载机制一致。

### 3.2 main.py

```python
# extensions/http_webhook/main.py
"""
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        fut = asyncio.run_coroutine_threadsafe(
            self.core.dispatch_event(event), self.core.loop)
        try:
...
class HttpWebhookAdapter(ProtocolAdapter):
    ...
    def start(self):
        cfg = self.core/config.get("http_webhook", {})
        host = cfg.get("host", "127.0.0.1")
        port = cfg.get("port", 8901)
        WebhookHandler.core = self.core
...
统一入口都是 `core.dispatch_event(event)`。**形态不同，插件零改动。**
...
- `ctx.send_msg / asend_msg`、`ctx.api / aapi` 已封装，不传 `bot` 时自动跟随当前来源
  （来源由 `core/runtime.py` 的 `current_source_var` 在 handler 执行期间注入）；
...
`core/protocol.py` 的 `ServiceRegistry` 是核心与插件的解耦点：
...
OneBot 协议的全部实现都在 `extensions/onebot_adapter/` 内，框架核心不含 OneBot 代码：
...
- 仍可用 `config.yaml → dual_process.extensions: [名字...]` 显式指定核心侧名单覆盖 meta；
- 判断逻辑见 `core/engine.py` 的 `_read_plugin_process_tag`（静态 AST 解析，不执行插件）。
