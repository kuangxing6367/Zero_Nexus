...
Zeronus 的内核（Framework）只负责：**加载插件、路由事件、提供公共服务**（数据库 / 权限 / 服务注册 / 事件总线）。
...
```
            ...
            │  加载 ─ 路由 ─ 服务注册 ─ 事件总线 ─ 数据库   │
            │                                             │
            │   lifecycle.startup / shutdown ──► 启动预热 / 资源释放
            │   http.before / after_request ──► 请求拦截 / 统一头
            │   event.before / after_dispatch ──► 事件过滤
            │   command.before / after ──► 命令切面
            │   message.before / after_send ──► 出站文本切面
            │   action.before / after ──► 协议动作切面
            │   service.register.before / after ──► 服务注册切面
            │   plugin.load / unload ──► 插件装卸切面
            │   db.query / execute.before / after ──► 数据库读写切面
            │   session.create / wait.before / after ──► 多轮会话切面
            │   cron.task.trigger.before / after ──► 定时任务切面
            └─────────────────────────────────────────────┘
              每一格都是一个扩展点，插件用 ctx.hook() 挂接（共 26 个）
```
...
| ---- | ------ | -------- | ------------ | -------- | ---------- |
| 生命周期 | `lifecycle.startup` | 全部插件加载注册完成后、终端启动前 | — | 否 | 事件循环（async） |
...
| 消息出站 | `message.before_send` | 框架主动发文本前（如权限不足提示、关键词回复） | `{'text','group_id','user_id','source'}` | 是（返回 `False` 取消发送） | 事件循环（async） |
| 消息出站 | `message.after_send` | 框架主动发文本后 | 同上 + `result` | 否 | 事件循环（async） |
| 协议动作 | `action.before` | 任意协议动作调用前（send_msg / 禁言 / 查询…） | `action(str), params(dict), bot` | 否（通知） | 视调用方（async/sync 皆可） |
| 协议动作 | `action.after` | 任意协议动作调用后 | `action, params, bot, result` | 否（通知） | 视调用方（async/sync 皆可） |
| 服务注册 | `service.register.before` | 服务注册进服务注册表前 | `name, service`（kwargs） | 否 | 视调用方 |
...
> 除表中标注「可短路」的 4 个外，其余均为**观察型（通知）**扩展点：handler 的返回值不影响主流程；
> 可短路点为 `http.before_request`、`event.before_dispatch`、`command.before`、`message.before_send`。
