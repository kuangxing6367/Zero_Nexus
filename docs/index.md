...
features:
  ...
  - title: 扩展点，处处可插
    details: 26 个标准扩展点覆盖生命周期、Web 请求、事件分发、命令执行、消息收发、协议动作、服务注册、插件装卸、数据库读写、多轮会话与定时任务触发；ctx.hook() 一行挂上去，做审计、限流或中间件。
...
在 `extensions.yaml` 开启官方插件 `http_inject` 后，任何系统都能推事件进来：
...
在 `extensions.yaml` 里开启 `http_inject`，用一条 `curl` 就能注入事件（见上方示例 3）。
