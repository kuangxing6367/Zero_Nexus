...
def host_entry(config_path, address, token, core_pid):
    ...
    from core.ipc.ipc_client import IpcClient

    client = IpcClient(address, token)
    client.connect()

    from core.engine import Framework
    from core.ipc.ipc_adapter import IpcAdapter
    ...
    from core.ipc.remote_route import RemoteRouteRegistry
    ...
    # 移除 root 的文件 handler，避免宿主与核心同时写同一 zernus.log 造成竞争。
    import logging
    from core.ipc.host_log import IpcLogHandler
