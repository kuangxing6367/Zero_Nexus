...
- 也可独立运行: python -m core.storage.init_db
"""
import logging
import os
import re
...
def _init_mysql(database):
    ...
    db_name = config.get('database', 'zernus')
...
def _init_sqlite(database):
    ...
    from .dialect import _translate_sql_for_sqlite
...
def _find_sql_file(filename: str) -> str:
    ...
    sql/ 位于项目根目录，故需从 core/storage 上溯到项目根。
    """
...
def _main():
    """独立运行: python -m core.storage.init_db [config.yaml]"""
    import sys
    from core.config import load_config
    from core.storage import init_db, _parse_sqlite_type
